...
function Perfil() {
  ...
  function modificarPerfil() {
    ...
    setAltura(perfil.altura === 0 ? "" : perfil.altura)
    setCuisine(perfil.cuisine)
    setSexo(perfil.sexo)
    setEdad(perfil.edad === 0 ? "" : perfil.edad)
  ...
  async function guardarCambios() {
    await fetch(`/api/modificar/perfil/${correo}`,
    {
      ...
      method: "PUT",
      body: JSON.stringify({"edad": parseInt(edad), "peso": parseFloat(peso), "altura": parseInt(altura), "indeseado": alergia.toString(), "alergia": health.toString(), "dieta": diet.toString(), "cocina_fav": cuisine.toString(), "sexo": sexo.toString() }),
    })
